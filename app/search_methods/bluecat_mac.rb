module Whowas
  class BluecatMac
    ...
    def input_formats
      {
        mac: lambda { |input| (input =~ /^([0-9A-F]{2}[:-]{0,1}){5}[0-9A-F]{2}$/i) 
                              && true rescue false }
      }      
    end
    ...
    def output_formats
      {
        # TODO: get log sample so I can determine how to extract the username
        # don't have working API credentials right now
      }
    end    
  end
end